...
**Phase to address:**
Phase 3 (batch processing core) — cancellation logic must be designed *before* processing is built, not retrofitted.
...
- **Tauri 2.x Event System** — Context7 (/websites/v2_tauri_app). `listen()` returns `Promise<UnlistenFn>`; unlisten required on listener scope exit; event names restricted to alphanumeric, `-`, `/`, `:`, `_`. Confidence: HIGH.
...
---
*Pitfalls research for: Tauri 2.x + Vue 3 + FFmpeg desktop video batch processor*
*Researched: 2026-05-12*
