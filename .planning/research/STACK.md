...
**Domain:** Desktop video batch processing tool (Tauri 2.x + Vue 3 + FFmpeg)
...
npm create tauri-app@latest sandwich -- --template vue-ts
cd sandwich

# Frontend dependencies
npm install pinia naive-ui

# Tauri plugins (frontend JS bindings)
npm install @tauri-apps/plugin-shell @tauri-apps/plugin-dialog @tauri-apps/plugin-fs

# Dev dependencies
npm install -D vitest @vue/test-utils happy-dom eslint prettier
```

```bash
# Rust dependencies (add to src-tauri/Cargo.toml)
cargo add ffmpeg-sidecar@2.5
...
```json
// src-tauri/tauri.conf.json — key plugin registration
{
  "plugins": {
    "shell": {
      "scope": [
...
**Primary approach: ffmpeg-sidecar `auto_download()`**

```
App Launch
  → ffmpeg_is_installed()?
    → YES: Use system/cached FFmpeg
    → NO: auto_download() to app data directory
      → Platform detection (macOS: aarch64/x86_64, etc.)
      → Download ~80MB from official FFmpeg builds
      → Extract to {app_data_dir}/ffmpeg/
      → Verify with ffmpeg_version()
```
...
**Fallback:** If user has system FFmpeg (via Homebrew on macOS, apt on Linux), ffmpeg-sidecar detects and uses it. `ffmpeg_is_installed()` checks PATH first.

## IPC Patterns

### Pattern 1: Request/Response (Rust Command)
For operations with defined inputs and outputs:
```rust
// Rust
#[tauri::command]
async fn generate_seed(seed_name: String) -> Result<SeedRecipe, String> {
    // Generate random operations chain
    Ok(SeedRecipe::random(&seed_name))
}
```
```typescript
// Vue
import { invoke } from '@tauri-apps/api/core';
const recipe = await invoke<SeedRecipe>('generate_seed', { seedName: 'My Seed' });
```

### Pattern 2: Event Streaming (Progress)
For long-running FFmpeg operations with progress updates:
```rust
// Rust: Emit progress events during FFmpeg processing
use tauri::Emitter;
app_handle.emit("ffmpeg-progress", ProgressPayload { frame, fps, percent })?;
```
```typescript
// Vue: Listen for progress events
...
For spawning FFmpeg as a child process:
```typescript
import { Command } from '@tauri-apps/plugin-shell';
const cmd = Command.create('ffmpeg', [
  '-i', inputPath, '-vf', filterGraph, '-y', outputPath
]);
cmd.stderr.on('data', (line) => { /* parse progress */ });
const child = await cmd.spawn();
```

## Stack Patterns by Variant

**If CPU-intensive FFmpeg processing:**
...
**If offline-first operation becomes a requirement:**
...
---
*Stack research for: Tauri 2.x + Vue 3 + FFmpeg desktop video batch processing tool*
*Researched: 2026-05-12*
