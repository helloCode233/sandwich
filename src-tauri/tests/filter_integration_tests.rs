...
use sandwich_lib::ffmpeg::filters::{
    MetadataContext, build_audio_channel_filter, build_audio_eq_filter, build_audio_pitch_filter,
    build_audio_resample_filter, build_audio_volume_filter, build_crop_filter,
    build_frame_drop_filter, build_metadata_selective_erase_filter, build_metadata_write_filter,
    build_trim_edges_filter, build_video_speed_filter,
};
...
fn generate_test_video() -> Result<PathBuf, String> {
    ...
    let output = dir.join("test_source_v2.mp4");
    ...
    let ffmpeg_str =
        if ffmpeg.exists() { ffmpeg.to_string_lossy().to_string() } else { "ffmpeg".to_string() };
    ...
    let status = Command::new(&ffmpeg_str)
        .args([
            ...
            "lavfi",
            "-i",
            "sine=frequency=440:duration=2,aformat=channel_layouts=stereo",
            "-c:v",
            "libx264",
            "-pix_fmt",
            "yuv420p",
    ...
    if status.success() { Ok(output) } else { Err("Failed to generate test video".to_string()) }
}
...
fn test_audio_resample_produces_valid_output() {
    ...
    assert!(
        out.exists() && out.metadata().map(|m| m.len() > 0).unwrap_or(false),
...
fn test_audio_channel_produces_valid_output() {
    ...
    for mode in &["swap", "mono", "stereo"] {
        ...
        let op = make_test_op(OperationType::AudioChannel, serde_json::json!({"mode": mode}));
        ...
        assert!(out.exists(), "Output file should exist for mode={}", mode);
        let _ = std::fs::remove_file(&out);
    }
}
...
fn test_crop_scale_restores_dimensions() {
    ...
    let args = build_crop_filter(&op, None, false).expect("build filter");
...
fn test_metadata_write_injects_fake_fields() {
    ...
    let ok =
        run_filter_on_test_video(&src.to_string_lossy(), &out.to_string_lossy(), &[], &[], &args)
            .expect("FFmpeg run");
...
fn test_metadata_selective_erase_keeps_non_targeted_fields() {
    ...
    run_filter_on_test_video(&src.to_string_lossy(), &tmp.to_string_lossy(), &[], &[], &write_args)
...
fn test_video_speed_produces_valid_output() {
    ...
    let args = build_video_speed_filter(&op, None, false).expect("build filter");
...
fn test_frame_drop_reduces_frame_count() {
    ...
    let status = Command::new(&ffmpeg_str)
        ...
        .arg(&vf_args[0])
        .arg("-vsync")
        .arg("vfr") // D-17: prevent duplicate frame insertion
        .arg("-c:v")
        .arg("libx264")
        .arg("-preset")
        .arg("ultrafast")
...
fn test_multiple_phase7_filters_chained() {
    ...
    let vs_args = build_video_speed_filter(&vs_op, None, false).expect("VideoSpeed");
    ...
    assert!(
        status.success(),
        "Chained Phase 7 filters (FrameDrop+Crop+VideoSpeed) should execute successfully"
    );
    assert!(
        out.exists() && out.metadata().map(|m| m.len() > 0).unwrap_or(false),
